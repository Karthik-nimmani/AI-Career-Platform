...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If session is null (email confirmation is enabled on Supabase)
      if (!data.session) {
        setSuccessMessage('Registration successful! Please check your email for a confirmation link.');
      } else {
        // Zustand auth listener will detect state and we redirect to dashboard
        router.push('/dashboard');
      }
    } catch (err: any) {
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center px-4 overflow-hidden">
      ...
      <div className="w-full max-w-md glass-panel rounded-2xl p-8 shadow-glass relative z-10 my-8">
        {/* Header */}
        <div className="text-center mb-8">
          ...
          <p className="text-muted mt-2">Get started with AI Career Intelligence</p>
        </div>

        {/* Success / Error Messages */}
        {successMessage && (
          <div className="p-4 rounded-xl bg-green-500/10 border border-green-500/20 text-green-400 mb-6 text-sm flex items-start gap-3">
        ...
        {!successMessage && (
          <>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1.5 text-gray-300" htmlFor="fullName">
                  Full Name
                </label>
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5 text-gray-300" htmlFor="password">
                  Password
                </label>
                <div className="relative">
              ...
              <div>
                <label className="block text-sm font-medium mb-1.5 text-gray-300" htmlFor="confirmPassword">
                  Confirm Password
                </label>
                <div className="relative">
                  <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-muted" />
              ...
              <button
                type="submit"
                disabled={localLoading}
                className="w-full bg-primary hover:bg-primary-hover text-primary-foreground font-semibold py-3 px-4 rounded-xl transition-all shadow-glow flex items-center justify-center gap-2 mt-4"
              >
                {localLoading ? (
                  <>
